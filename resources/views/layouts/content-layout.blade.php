...
{{--                    <svg style="margin-top: 4px;" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50" width="24" height="24"><path d="M 20 4 C 15.054545 4 11 8.0545455 11 13 L 11 35.585938 L 5.7070312 30.292969 L 4.2929688 31.707031 L 12 39.414062 L 19.707031 31.707031 L 18.292969 30.292969 L 13 35.585938 L 13 13 C 13 12.759091 13.01313 12.521884 13.037109 12.287109 C 13.396795 8.7654918 16.386364 6 20 6 L 30 6 L 30.5 6 L 30.5 4 L 30 4 L 20 4 z M 38 10.585938 L 30.292969 18.292969 L 31.707031 19.707031 L 37 14.414062 L 37 37 C 37 40.613636 34.234508 43.603205 30.712891 43.962891 C 30.478116 43.98687 30.240909 44 30 44 L 20 44 L 19.5 44 L 19.5 46 L 20 46 L 30 46 C 30.309091 46 30.614657 45.984029 30.916016 45.953125 C 35.436391 45.489569 39 41.636364 39 37 L 39 14.414062 L 44.292969 19.707031 L 45.707031 18.292969 L 38 10.585938 z"/></svg>--}}
                    <img width="28" alt="trade" src="{{ asset('images/footer-trade.png') }}" class="let-foot-trade">
                    <!--<span>Trade</span>-->
                </div>
            </a>
{{--            <a href="{{ route('home') }}"--}}
...
{{--            </a>--}}
            <a href="{{ route('home') }}" class="nav-home middle text-decoration-none  bottom-nav-logo {{ request()->fullUrl() === route('home') ? 'active' : ''}}">
                <div class="bottom-nav-logo d-flex justify-content-center flex-column align-items-center">
                    <img  src="{{asset('images/homes.png')}}" alt="home" width="28" height="28">
                </div>
            </a>
...
{{--                    <svg width="24" height="24" fill="none" xmlns="http://www.w3.org/2000/svg"><title>Profile</title><path fill-rule="evenodd" clip-rule="evenodd" d="M4.688 4.5h14.625c.103 0 .187.084.187.188v14.625a.187.187 0 01-.188.187H4.688a.187.187 0 01-.187-.188V4.688c0-.103.084-.187.188-.187zM3 4.688C3 3.756 3.756 3 4.688 3h14.625C20.244 3 21 3.756 21 4.688v14.625c0 .931-.756 1.687-1.688 1.687H4.688A1.687 1.687 0 013 19.312V4.688zm8.938 9.687a2.437 2.437 0 100-4.875 2.437 2.437 0 000 4.875zm3.937-2.438a3.939 3.939 0 01-3.153 3.86.752.752 0 01.028.203v.75a.75.75 0 01-1.5 0V16a.75.75 0 01.022-.181 3.939 3.939 0 114.603-3.882z" fill="currentColor"></path></svg>--}}
                    <img width="27" alt="trade" src="{{ asset('images/profile.png') }}">
                    <!--<span>Profile</span>-->
                </div>
            </a>
            {{--@can('manage_users')
...
document.addEventListener('DOMContentLoaded', function() {
    var listItems = document.querySelectorAll('.col-12.d-flex.gap-2');
    var boxes = document.querySelectorAll('.col-12.d-flex.gap-2 + .info.mt-1.text-white');

    // Make the first item active initially
    if (listItems.length > 0 && boxes.length > 0) {
    ...
    listItems.forEach(function(item, index) {
        item.addEventListener('click', function() {
            // Remove the 'active' class from all listItems and boxes
            ...
            // Add the 'active' class to the clicked listItem and the corresponding box
	...
	document.addEventListener('DOMContentLoaded', function() {
      ...
      buttons.forEach(button => {
        button.addEventListener('click', function() {
          ...
          const allContents = document.querySelectorAll('.accordion-item .accordion-content');
          ...
          // Toggle 'active' class on the clicked button and content section
...
document.addEventListener('DOMContentLoaded', function() {
    // Select the last child anchor element in the specified container
    var lastChildAnchor = document.querySelector('.menu-buttons-container .menu-buttons.row:nth-of-type(2) .polygon-container a:last-child');
    console.log("Selected lastChildAnchor:", lastChildAnchor);
    
    // Select the target container where the element will be moved
    var targetContainer = document.querySelector('.menu-buttons-container .menu-buttons.row:nth-of-type(3) .polygon-container');
    console.log("Selected targetContainer:", targetContainer);
    
    // Append the selected element to the target container as the first child
    if (lastChildAnchor && targetContainer) {
        // Insert the element before the first child of the target container
        targetContainer.insertBefore(lastChildAnchor, targetContainer.firstChild);
        console.log("Element moved successfully");
    } else {
        console.log("Element or target container not found");
    }
});

document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.let-withrow-amount-ss .let-i').forEach(function(element) {
        element.addEventListener('click', function() {
            document.querySelectorAll('.lt-popup-text').forEach(function(popupTextElement) {
                popupTextElement.classList.toggle('active');
            });
        });
    });
});
